...
public class RPiBluetoothService {
    ...
    private synchronized void setState(int state) {
        mState = state;
        mHandler.obtainMessage(getNewDataActivity.MESSAGE_STATE_CHANGE, state, -1).sendToTarget();
    }
    ...
    public void queryTime(int year, int month, int day,int hour,int minute,int second) {
        JSONObject req = new JSONObject();
        ...
        try {
            ...
            req.put("day", day);
            req.put("hour", hour);
            req.put("minute", minute);
            req.put("second", second);
        }
        catch (JSONException e) {
            e.printStackTrace();
        }
        Log.d("queryTime:", req.toString());
        write((req.toString() + "\n").getBytes());
    }
    ...
    private void connectionFailed() {
        ...
        Message msg = mHandler.obtainMessage(getNewDataActivity.MESSAGE_TOAST);
        Bundle bundle = new Bundle();
        bundle.putString(getNewDataActivity.TOAST, "Unable to connect device");
        msg.setData(bundle);
    ...
    private void connectionLost() {
        ...
        Message msg = mHandler.obtainMessage(getNewDataActivity.MESSAGE_TOAST);
        Bundle bundle = new Bundle();
        bundle.putString(getNewDataActivity.TOAST, "Device connection was lost");
        msg.setData(bundle);
        mHandler.sendMessage(msg);
    }
    ...
    private class ConnectedThread extends Thread {
            ...
            while (true) {
                try {
                    ...
                    if (buffer[bytes - 1] == '\n') {
                        ...
                        mHandler.obtainMessage(getNewDataActivity.MESSAGE_READ, message).sendToTarget();
                        message = "";
                    }
        ...
        public void write(byte[] buffer) {
            try {
                ...
                mHandler.obtainMessage(getNewDataActivity.MESSAGE_WRITE, -1, -1, buffer)
                        .sendToTarget();
            } catch (IOException e) {
            }
        }
