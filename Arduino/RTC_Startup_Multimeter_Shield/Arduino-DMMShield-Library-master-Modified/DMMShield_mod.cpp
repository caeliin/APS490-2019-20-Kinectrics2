...
**
**	Parameters:
**		SoftwareSerial *phwSerial	- a pointer to SoftwareSerial object, initialized the caller (normally main sketch). 
...
**		The function receives as parameter a pointer to the SoftwareSerial object. The SoftwareSerial object must be already initialized by the caller. 
**		This pointer to SoftwareSerial is provided when calling the initialization function of DMMCMD module.
*/
void DMMShield_mod::begin(SoftwareSerial *phwSerial)
{
...
**		Basically the command interpreter checks if there is any available received text over SoftwareSerial object (Serial Monitor). 
**		If so, it identifies a list of commands and runs the specific functionality for each command.
*/
void DMMShield_mod::CheckForCommand()
{
...
*/
void DMMShield_mod::ProcessIndividualCmd(char *szCmd)
{
...
**            
*/
uint8_t DMMShield_mod::SetScale(int idxScale)
{
...
**		In case of error, the error specific message is sent over UART.
*/
uint8_t DMMShield_mod::GetFormattedValue(char *pString)
{
	uint8_t bErrCode;
	double dMeasuredVal;
